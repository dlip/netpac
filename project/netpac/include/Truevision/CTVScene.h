...
public:
	...
	CTVRenderSurface* CreateRenderSurface(int iWidth, int iHeight, bool bUseDepthBuffer = true, cCONST_TV_RENDERSURFACEFORMAT iFormat = cTV_TEXTUREFORMAT_DEFAULT);
	...
	CTVRenderSurface* CreateAlphaRenderSurface(int iWidth, int iHeight, bool bUseDepthBuffer = true);
 //	int LoadTexture(char* sFilename, int iWidth = -1, int iHeight = -1, char* sTextureName = "");
	...
	CTVRenderSurface* CreateCubeRenderSurface(int iSize,  bool bUseDepthBuffer = true, int iFormat = 0);
 
	void RenderAllMeshes(bool bAlphaSort = false);
	void RenderAll(bool bAlphaSort); 
	void RenderMeshList(int iNumMeshs, int* pMeshIndicesListArray);

	void SetCursor(bool bEnable, int iTextureCursor, int iHotSpotX = 0, int iHotSpotY = 0, float fOptionalSizeX = 0.0f, float fOptionalSizeY = 0.0f);
	CTVNode* CreateNode(const char* sName); 
	...
	CTVRenderSurface* CreateRenderSurfaceEx(int iWidth, int iHeight, cCONST_TV_RENDERSURFACEFORMAT eFormat, bool bUseDepth, bool bUseMainBufferSize = false, float fMainBufferScale = 1.0f);
	...
	CTVPath* CreatePath();
	CTVAI* CreateAI();
	CTVLandscape* CreateLandscape(const char* sName);
